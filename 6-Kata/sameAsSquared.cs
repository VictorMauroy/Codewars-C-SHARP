...
class AreTheySame
{
  public static bool comp(int[] rootArr, int[] squaredArr)
  {
    if(rootArr is null || squaredArr is null || rootArr.Length != squaredArr.Length)
      return false;
    
    List<int> sortedRoots = rootArr.OrderBy(num => num).ToList();
    List<int> sortedSquares = squaredArr.OrderBy(num => num).ToList();
    
    for(int i = 0; i < sortedRoots.Count; i++)
      if(sortedRoots[i] * sortedRoots[i] != sortedSquares[i])
        return false;
