...
an index N where the sum of the integers to the left of N is equal to the sum of the integers to the right of N. 
...
public class Kata
{
  public static int FindEvenIndex(int[] arr)
  {
     for(int i = 0; i < arr.Length; i++){
      int leftSum = arr[0..i].Aggregate(
        0,
        (acc, num) => acc + num
       );
      
      int rightSum = arr[(i+1)..arr.Length].Aggregate(
        0,
        (acc, num) => acc + num
       );
